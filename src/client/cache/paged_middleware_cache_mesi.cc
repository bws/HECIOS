...
void PagedMiddlewareCacheMesi::handleFileSystemMessage(cMessage* msg)
{
    ...
    if (SPFS_MPI_FILE_READ_AT_RESPONSE == responseKind ||
        ...
        SPFS_CACHE_READ_EXCLUSIVE_RESPONSE == responseKind ||
        SPFS_CACHE_READ_SHARED_RESPONSE == responseKind)
    {
    ...
    else
    {
        ...
        assert(SPFS_CACHE_READ_EXCLUSIVE_RESPONSE != responseKind);
        assert(SPFS_CACHE_READ_SHARED_RESPONSE != responseKind);
        assert(SPFS_CACHE_INVALIDATE_RESPONSE != responseKind);
        assert(SPFS_CACHE_SEND_PAGES != responseKind);
